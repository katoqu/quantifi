...
def _expand_strength_sets_into_rows(df):
    ...
    for idx, row in df.iterrows():
        sets = row.get("sets")
        # Handle None, empty list, or string representation
        if sets is None:
            sets = []
        elif isinstance(sets, str):
            # Try to parse string representation of sets (e.g., from JSON)
            import json
            try:
        ...
        if isinstance(sets, list) and len(sets) > 0:
            ...
            if pd.isna(original_id) or original_id is None:
                # Fallback to using original_id if id is not available
                original_id = row.get("original_id", idx)
            
            for set_idx, set_data in enumerate(sets):
                if isinstance(set_data, dict):
                    set_row = row.copy()
                    set_row["original_id"] = original_id  # Track original database entry
            ...
            # No sets data or empty sets, keep as single row
            # But check if we have load_kg and reps_per_set to create at least one set
            row_copy = row.copy()
            original_id = row.get("id")
            if pd.isna(original_id) or original_id is None:
                original_id = row.get("original_id", idx)
            row_copy["original_id"] = original_id
...
def _aggregate_expanded_sets_back(df):
    ...
    for original_id, session_df in session_groups:
        ...
        for _, set_row in session_df.iterrows():
            set_data = {
                "load_kg": set_row.get("load_kg"),
                "reps": set_row.get("reps_per_set", set_row.get("reps", 1))
            }
...
def execute_save(mid, state_key, editor_key):
    ...
    for _, row in df_to_process[df_to_process["Change Log"] == "🟡"].iterrows():
        if pd.notna(row.get("id")):
            ...
            # For aggregated sessions (expanded sets), use the sets array directly
            if "original_id" in df_to_process.columns and "sets" in row and isinstance(row.get("sets"), list):
                payload["sets"] = row["sets"]
                payload["load_kg"] = row.get("load_kg")
                payload["value"] = payload["load_kg"] if payload["load_kg"] is not None else db_val
            else:
                # For non-aggregated strength data or regular metrics
    ...
    # Remove all editor keys to force fresh rendering
    keys_to_remove = [key for key in st.session_state.keys() if key.startswith(f"{editor_key}_") or key == editor_key]
    for key in keys_to_remove:
        st.session_state.pop(key, None)
    reset_editor_state(state_key, mid)
    if fresh_dfe is not None:
        # IMPORTANT: For strength sessions, we need to expand the sets into individual rows
        # Use prepare_entry_editor_frame to ensure proper expansion
        # Check if this is a strength session by looking at columns or data structure
        is_strength = False
        if "load_kg" in fresh_dfe.columns or "sets" in fresh_dfe.columns:
            is_strength = True
        elif "reps_per_set" in fresh_dfe.columns or "set_count" in fresh_dfe.columns:
            is_strength = True
        elif "original_id" in fresh_dfe.columns or "set_index" in fresh_dfe.columns:
            is_strength = True
        
        metric_kind = "strength_session" if is_strength else None
        prepared_df = prepare_entry_editor_frame(fresh_dfe, metric_kind=metric_kind)
        st.session_state[f"saved_data_{mid}"] = prepared_df.copy()
        st.session_state[state_key] = prepared_df.assign(**{"Change Log": "", "Select": False})
